...
You are to create an AI powered workflow/automated AI process. The specific implementation and how you solve it is up to you. The solution needs to fulfill the requirements listed below.

> [!IMPORTANT]
> GitHub now have greater support for using GitHub Copilot in actions and gh aw (agentic workflows), one no longer need to add a seperate secret in the actions secrets, so that setting the workflow permissions should be enough. 
> Due to the nature of LLMs the agent might reccomend and implement a version that doesn't take the latest changes into consideration. Be sure to double check this and instruct the agent to first look up current state of these tools.
> You might still get some errors in the first run, and using AI to debug it might jump to the conclusion that it lacks access to copilot (and tries to change the implementation to rely on a secret variable) but the actual cause of the error might be it exceeding the max AI credits consumed and/or the labels not existing yet on the repository in GitHub (the agent doesn't neccesarily tell you to create them and expects them to be in place)
