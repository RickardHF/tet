...
> [!TIP]
> To test the changes introduce some errors to the agents in this repo and run the evaluation and check the result
> ```
> cd evaluator
> npm run --silent cli -- evaluate -d .. --json > eval_results.jsonl 2> eval_errors.txt
> ```

## Tasks

Update the evaluator tool scripts to add additional programatic checks and prompt changes in the existing functions so that the evaluation is more precise. Implement the requirements listed below.
...
> [!TIP]
> Using AI to create these changes is completely fine
> Lead with using the 'plan' mode
