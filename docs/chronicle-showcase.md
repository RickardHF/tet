...
Available across Copilot surfaces since 2 June 2026.

## What Chronicle actually is

Copilot CLI and GitHub Copilot app sessions are recorded locally. Other session types use the
storage provided by their host, and Copilot cloud agent sessions run on GitHub.

| Where | What |
|---|---|
| `~/.copilot/session-state/` | Complete local records for Copilot CLI and GitHub Copilot app sessions |
| `~/.copilot/session-store.db` | Local SQLite index that `/chronicle` queries |
| GitHub account (sync) | Local CLI and Copilot app sessions sync by default when policy allows, so they can appear alongside other agent sessions |
| GitHub.com | Copilot cloud agent session logs remain available after their ephemeral execution environment is destroyed |

Local sessions are **unshared by default**, and synced copies are tied to your account. Admins
control *whether* syncing is available via the "Store local sessions in the Cloud" policy, but
enabling that policy does not give them access to your synced session data. Copilot cloud agent
sessions are different: they are shared by default with people who can access the repository.
...
2. **Open Copilot CLI** in this repo (the CLI is where all subcommands exist; other surfaces
   expose a subset).
3. **Run `/chronicle reindex` once** before you present, so the local store is rebuilt from the
   local session files and synced when account sync is enabled.
...
Analyzes friction signals in the current repo and proposes additions to
...
- **`/share gist`** — share a view-only copy of a single session. Useful for "look at what this
  agent did" without granting account access. Gist sharing is unavailable to Enterprise Managed
  Users and GitHub Enterprise Cloud accounts with data residency.
...
- Local CLI and Copilot app sessions are **unshared by default**, including after they sync to
  your account.
- Local session sync is enabled by default when policy allows. Set `"remoteExport": false` in
  `~/.copilot/settings.json` to stop exporting new local session data.
- On Copilot Business/Enterprise, the "Store local sessions in the Cloud" policy must be set to
  at least **View from cloud** before local sessions can sync.
- Copilot cloud agent sessions are **shared by default** with people who have repository access;
  do not describe those sessions as private local history.
...
naturally next to [Exercise 5 — Pitfalls & Tips](../exercises/GitHub%20Copilot%20Customization%20101/05-pitfalls-quiz.md),
